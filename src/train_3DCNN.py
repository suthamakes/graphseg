...
class Net3D(nn.Module):
    """
    3D CNN model for joint spectral-spatial hyperspectral image classification.

    Input shape : (N, 1, B, P, P)
      1  = single input channel
      B  = spectral bands (treated as the depth / D dimension)
      P  = spatial patch side length

    The 3D convolution kernel slides simultaneously over the spectral depth
    and both spatial dimensions, learning local spectral-spatial correlations
    that a 1D (spectral-only) or 2D (spatial-only) kernel cannot capture.

    Architecture
    ------------
    Block 1 : Conv3d(1,  8,  kernel=(7,3,3)) -> BN3d -> ReLU -> Dropout3d
    Block 2 : Conv3d(8,  16, kernel=(5,3,3)) -> BN3d -> ReLU -> Dropout3d
    Block 3 : Conv3d(16, 32, kernel=(3,1,1)) -> BN3d -> ReLU
    Head    : GlobalAveragePool3d -> Linear(32, num_classes)

    Kernel choices
    - Large spectral kernel (7/5/3) captures broad spectral correlations.
    - Small spatial kernel (3x3 then 1x1) focuses on local spatial context.
    - Padding keeps spatial dimensions intact; spectral depth shrinks each block.
    """

    def __init__(self, in_channels=1, num_classes=16, dropout=0.4):
        ...
        # Global average pool collapses (N, 32, D, H, W) -> (N, 32)
    ...
    def forward(self, x):
        """Forward pass. x: (N, 1, B, P, P) -> logits: (N, num_classes)."""
...
def initialize_parameters(in_channels=1, num_classes=16, dropout=0.4):
    """
    Initialise the 3D CNN with a fixed seed for reproducibility.

    Returns:
        Net3D: Initialized model instance.
    """
    torch.manual_seed(1)
    model = Net3D(in_channels=in_channels, num_classes=num_classes, dropout=dropout)
    return model


class dataset(Dataset):
    """PyTorch Dataset for 3D CNN patch-based HSI classification."""

    def __init__(self, data_dir='data/extracted_3d', is_train=True, transform=None, logger=None):
        ...
        self.data   = np.load(data_path)    # (N, 1, B, P, P)
    ...
    def __getitem__(self, idx):
        x = self.data[idx]     # (1, B, P, P) -- already channel-first 3D volume
...
def plot_predictions(
    ...
    output_plot_path='results/plots/segmentation_comparison_3d.png',
    logger=None,
):
    """
    Run full-image inference with the trained 3D CNN and save a 3-panel plot:
      [False-colour composite | Ground Truth | 3D CNN Predicted Classes].
    """
    model.eval()
    ...
    # 4. Build all-pixel patch tensor (H*W, 1, B, P, P)
    all_patches = []
    for r in range(rows):
        for c in range(cols):
            pr, pc = r + half, c + half
            ...
            # Transpose to (B, P, P) then add channel dim -> (1, B, P, P)
            patch_3d = patch.transpose(2, 0, 1)[np.newaxis, ...]
            all_patches.append(patch_3d)

    all_patches = np.array(all_patches, dtype=np.float32)      # (H*W, 1, B, P, P)
    x_tensor    = torch.tensor(all_patches)

    # 5. Predict in batches
    predictions = []
    batch_size  = 256   # smaller batch due to larger tensor size
    with torch.no_grad():
    ...
    axes[2].set_title("3D CNN Predicted Classes")
    axes[2].axis('off')
...
def main():
    ...
    EPOCHS      = 100
    LR          = 0.001
    BATCH_SIZE  = 32    # smaller than 1D/2D due to larger 3D input volumes
    DROPOUT     = 0.4
    TEST_SPLIT  = 0.5
    PATCH_SIZE  = 5
    IN_CHANNELS = 1     # Conv3d input channels (single channel, spectral=depth)
    NUM_CLASSES = 16
    # --------------------------------------------------------

    # Setup logging using shared helper function
    logger = setup_logger(log_dir='results', log_filename='training_3d.log')
    ...
    csv_path = log_experiment(params, metrics, log_dir='results', csv_filename='experiments_3d.csv')
