...
def mat2gray_normalize(data_cube):
    ...
    # Compute per-band stats (axis=(0,1) -> spatial axes)
...
# 7. Transpose patches to channel-first format for PyTorch Conv2d: (N, B, P, P)
#    Current shape after extraction: (N, P, P, B)
Train_X_ready = np.transpose(Train_X, (0, 3, 1, 2))   # (N, B, P, P)
Test_X_ready  = np.transpose(Test_X,  (0, 3, 1, 2))   # (N, B, P, P)

print("\nSaving finalized arrays to extracted_2d folder...")
...
print(f"\nFinal shapes:")
print(f"  Train_X : {Train_X_ready.shape}  (N, Bands, {PATCH_SIZE}, {PATCH_SIZE})")
